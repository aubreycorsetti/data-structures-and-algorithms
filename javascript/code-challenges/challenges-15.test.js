...
const screenForNames = (arr) => {
  // Solution code here...
  let name = /^(Mr. |Mrs. |Ms. |Dr. )[A-Za-z]*$/;
  return arr.filter(str => name.test(str));
};
...
const toTitleCase = (arr) => {
  // Solution code here...
  let item = /^[a-z]/;
  return arr.map(str => str.replace(item, str.charAt(0).toUpperCase()));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.filter(per => per.mass > 77 ).reduce((names, per) => names +' - ' + per.name,'').replace(' - ','');
};
