...
const longestString = (arr) => {
  // Solution code here...
  let longString = arr.reduce((longest, str) => longest.length > str.length ? longest : str, '');
  return arr.findIndex(string => string === longString);
};
...
const firstLetters = (arr) => {
  // Solution code here...
  return arr.map(str => str.slice(0,1));
};
...
const findHappiness = (arr) => {
  // Solution code here...
  let happy = /[:][)]/;
  return arr.filter(str => happy.test(str));
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(str => str.slice(1,4)+str.slice(6,9)+str.slice(10,14));
};
...
const onlyOddChars = (str) => {
  ...
  if(str.length<=1){
    ...
    for(let i = 1;i<=str.length;i=i+2){
      if(i===str.length){
        oddChart = oddChart + str.slice(-1);
      }else{
        oddChart = oddChart + str.slice(i, i+1);
      }
    }
    return oddChart;
  }
...
const allHappy = (arr) => {
  // Solution code here...
  let happy = /[:][)]/;
  return arr.map(str => happy.test(str)).includes(false) ? false : true;
};
